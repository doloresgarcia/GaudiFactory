...
def build_executor_prompt(
    *,
    ...
    parts = [
        ...
        "",
        "=== BLOCKED? REDIRECT, DON'T STOP ===",
        "If a resource is unavailable (missing MC, no GPU, insufficient stats,",
        "inaccessible data), downscope to the best achievable method — but DO NOT",
        "STOP THERE. A blocker on one path means redirect effort to parallel work:",
        "- Measure every observable the data supports, not just the primary one.",
        "- Cross-check exhaustively (year-by-year, subdetector, charged-only).",
        "- Consult literature (RAG) for published corrections or results to compare.",
        "- Build infrastructure for the blocked steps (write the code with placeholders",
        "  so when the resource arrives, it's one session to finish).",
        "The test: if the blocker clears tomorrow, can the next session finish in one",
        "sitting? If not, you stopped too early. Infrastructure is a deliverable.",
        "",
        "=== PHASE INSTRUCTIONS ===",
